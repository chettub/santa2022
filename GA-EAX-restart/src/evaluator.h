...
public:
    ...
    bool checkValid(ll* array, int128 value);                    /* Check an tour */
    int128 funcEdgeDis(int i, int j);                            /* funcEdgeDis[i][j]: distance between i and j */
    int128 funcCostConstraintViolation(const TIndi& indi) const; /* Number of constraint violation times*/

    ll fNearNumMax;    /* Maximum number of k (see below) */
    ll** fNearCity;    /* NearCity[i][k]: k-th nearest city from */
    int128** fEdgeDis; /* EdgeDis[i][j]: distance between i and j */
