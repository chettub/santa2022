...
TKopt::TKopt(ll N) {
    ...
    fT = new ll[5];
    fActiveV = new ll[fN];
    fInvNearList = new ll*[fN];
    for (ll i = 0; i < fN; ++i)
        fInvNearList[i] = new ll[500];
...
void TKopt::transIndiToTree(TIndi& indi) {
    ...
    while (1) {
        ...
        ++num;
        ++size;
        for (ll i = 0; i < (ll)sqrt(fN * 1.0) - 1; ++i) {
            if (num == fN)
                break;
...
void TKopt::swap(ll& x, ll& y) {
    ll s = x;
    x = y;
    y = s;
}

ll TKopt::turn(ll& orient) {
    return 1 - orient;
}

void TKopt::incrementImp(ll flagRev) {
    ll t1_s, t1_e, t2_s, t2_e;
    ll seg_t1_s, seg_t1_e, seg_t2_s, seg_t2_e;
    ll ordSeg_t1_s, ordSeg_t1_e, ordSeg_t2_s, ordSeg_t2_e;
    ll orient_t1_s, orient_t1_e, orient_t2_s, orient_t2_e;
    ll numOfSeg1, numOfSeg2;
    ll curr;
    ll ord;
    ...
    if ((seg_t1_s == seg_t1_e) && (seg_t1_s == seg_t2_s) && (seg_t1_s == seg_t2_e)) {
        if ((fOrient[seg_t1_s] == 1 && (fOrdCity[t1_s] > fOrdCity[t1_e])) || (fOrient[seg_t1_s] == 0 && (fOrdCity[t1_s] < fOrdCity[t1_e]))) {
            this->swap(t1_s, t2_s);
            this->swap(t1_e, t2_e);
    ...
    if (fLink[t2_e][orient_t2_e] == -1)
        flag_t2e_t1s = 1;
    else
        flag_t2e_t1s = 0;

    if (fLink[t2_s][this->turn(orient_t2_s)] == -1)
        flag_t2s_t1e = 1;
    else
        flag_t2s_t1e = 0;
    ...
    while (fNumOfSeg > fFixNumOfSeg) {
        if (fSizeSeg[fLinkSeg[fNumOfSeg - 1][0]] < fSizeSeg[fLinkSeg[fNumOfSeg - 1][1]])
...
void TKopt::combineSeg(ll segL, ll segS) {
    ll seg;
    ll t_s, t_e, direction;
    t_s = 0;
    t_e = 0;
    direction = 0;
    ll ord;
...
void TKopt::checkValid() {
    ll t_st, t_c, t_n, count, seg, orient, Invalid = 0;
    for (ll i = 0; i < fN; ++i)
        fCheckN[i] = 0;
